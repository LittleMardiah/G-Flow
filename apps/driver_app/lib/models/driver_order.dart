...
class DriverOrder {
  ...
  factory DriverOrder.fromRideJson(Map<String, dynamic> j, {bool isMock = false}) {
    return DriverOrder(
      ...
      deliveryAddress: _str(j['dropoff_address'] ?? j['dropoff_label']) ?? '',
      deliveryLat: _num(j['dropoff_lat']),
      deliveryLng: _num(j['dropoff_lng']),
      distanceKm: _num(j['distance_km']),
      estimatedFare: _int(j['estimated_fare'] ?? j['fare']),
      driverEarning: _int(j['driver_earning'] ?? j['earning'] ?? (j['estimated_fare'] is num ? (j['estimated_fare'] as num) * 0.8 : 0)),
  ...
  factory DriverOrder.fromFoodJson(Map<String, dynamic> j, {bool isMock = false}) {
    ...
    return DriverOrder(
      ...
      driverId: _str(j['driver_id']),
      pickupAddress: _str(j['merchant_address'] ?? merchant['address'] ?? j['pickup_address']) ?? '',
      pickupLat: _num(j['merchant_lat'] ?? merchant['location_lat']),
      pickupLng: _num(j['merchant_lng'] ?? merchant['location_lng']),
      deliveryAddress: _str(j['delivery_address'] ?? j['dropoff_label']) ?? '',
      deliveryLat: _num(j['delivery_lat'] ?? j['dropoff_lat']),
      deliveryLng: _num(j['delivery_lng'] ?? j['dropoff_lng']),
      merchantName: _str(j['merchant_name'] ?? merchant['name'] ?? j['pickup_address']) ?? '',
      merchantAddress: _str(j['merchant_address'] ?? merchant['address']) ?? '',
      distanceKm: _num(j['distance_km']),
      estimatedFare: _int(j['total_amount'] ?? j['delivery_fee'] ?? j['fare']),
      driverEarning: _int(j['driver_earning'] ?? j['earning'] ?? j['delivery_fee']),
      paymentMethod: _str(j['payment_method']) ?? 'WALLET',
  ...
  factory DriverOrder.fromSendJson(Map<String, dynamic> j, {bool isMock = false}) {
    ...
    return DriverOrder(
      ...
      pickupLat: _num(j['pickup_lat']),
      pickupLng: _num(j['pickup_lng']),
      deliveryAddress: stops.isEmpty ? (_str(j['first_stop_address'] ?? j['dropoff_label']) ?? '') : stops.first.address,
      ...
      estimatedFare: _int(j['total_fare'] ?? j['total_amount'] ?? j['fare']),
      driverEarning: _int(j['driver_earning'] ?? j['earning']),
      paymentMethod: _str(j['payment_method']) ?? 'WALLET',
      createdAt: _date(j['created_at']),
      stops: stops,
      isMock: isMock,
